--[[
	...
	Ryzee Interface Suite
	by ig: @Oreeo.reo
]]

if debugX then
	warn('Initialising Ryzee')
end
...
local requestsDisabled = true --getgenv and getgenv().DISABLE_Ryzee_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local RyzeeFolder = "Ryzee"
local ConfigurationFolder = RyzeeFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		RyzeeOpen = {Type = 'bind', Value = 'K', Name = 'Ryzee Keybind'},
		-- buildwarnings
		-- Ryzeeprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.RyzeeOpen"] = "J"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(RyzeeFolder) then
				if isfile and isfile(RyzeeFolder..'/settings'..ConfigurationExtension) then
					file = readfile(RyzeeFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"RyzeeOpen":{"Value":"K","Type":"bind","Name":"Ryzee Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Ryzee Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Ryzee had an issue accessing configuration saving capability.')
		end
	end
end
...
local Ryzee = useStudio and script.Parent:FindFirstChild('Ryzee') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local RyzeeDestroyed = false -- True when RyzeeLibrary:Destroy() is called

repeat
	if Ryzee:FindFirstChild('Build') and Ryzee.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Ryzee | Build Mismatch')
		print('Ryzee may encounter issues as you are running an incompatible interface version ('.. ((Ryzee:FindFirstChild('Build') and Ryzee.Build.Value) or 'No Build') ..').\n\nThis version of Ryzee is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Ryzee = Ryzee, useStudio and script.Parent:FindFirstChild('Ryzee') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(Ryzee)
	Ryzee.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Ryzee.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Ryzee.Name and Interface ~= Ryzee then
			Interface.Enabled = false
			Interface.Name = "Ryzee-Old"
		end
	end
elseif not useStudio then
...
if Ryzee.AbsoluteSize.X < minSize.X and Ryzee.AbsoluteSize.Y < minSize.Y then
...
local Main = Ryzee.Main
local MPrompt = Ryzee:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Ryzee:FindFirstChild('Drag')
...
Ryzee.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/Ryzee/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = Ryzee.Notifications

local SelectedTheme = RyzeeLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = RyzeeLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Ryzee.Main.BackgroundColor3 = SelectedTheme.Background
	Ryzee.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Ryzee.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Ryzee.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Ryzee.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Ryzee.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Ryzee.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Ryzee.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Ryzee.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Ryzee:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Ryzee | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Ryzee | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Ryzee had an issue decoding the configuration file, please try delete the file and reopen Ryzee.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(RyzeeLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Ryzee | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--RyzeeLibrary:Notify({Title = "Ryzee Flags", Content = "Ryzee was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(RyzeeLibrary.Flags) do
...
function RyzeeLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Ryzee | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RyzeeLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			RyzeeLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "RyzeeOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(RyzeeFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Ryzee Settings', 0, true)

	if TabList['Ryzee Settings'] then
		TabList['Ryzee Settings'].LayoutOrder = 1000
	end

	if Elements['Ryzee Settings'] then
		Elements['Ryzee Settings'].LayoutOrder = 1000
	end
...
function RyzeeLibrary:CreateWindow(Settings)
	print('creating window')
	if Ryzee:FindFirstChild('Loading') then
		if getgenv and not getgenv().RyzeeCached then
			Ryzee.Enabled = true
			Ryzee.Loading.Visible = true

			task.wait(1.4)
			Ryzee.Loading.Visible = false
		end
	end

	if getgenv then getgenv().RyzeeCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				RyzeeLibrary:Notify({Title = 'Build Mismatch', Content = 'Ryzee may encounter issues as you are running an incompatible interface version ('.. ((Ryzee:FindFirstChild('Build') and Ryzee.Build.Value) or 'No Build') ..').\n\nThis version of Ryzee is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Ryzee"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Ryzee Interface Suite" then
		LoadingFrame.Version.Text = "Ryzee UI"
	end
	...
	if not Settings.DisableRyzeePrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				RyzeeLibrary:Notify({
					Title = "Ryzee Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(RyzeeFolder.."/Discord Invites") then
			makefolder(RyzeeFolder.."/Discord Invites")
		end

		if isfile and not isfile(RyzeeFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(RyzeeFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Ryzee RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(RyzeeFolder.."/Key System") then
			makefolder(RyzeeFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Ryzee | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Ryzee specific development.')
				end
			end
		end
		...
		if isfile and isfile(RyzeeFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(RyzeeFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(RyzeeFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						RyzeeLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called RyzeeLibrary:Destroy()
				if RyzeeDestroyed then
					return
				end
				if not Success then
					...
					print("Ryzee | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Ryzee specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= RyzeeLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					RyzeeLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Ryzee.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Ryzee | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Ryzee specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					RyzeeLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Ryzee | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Ryzee specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					RyzeeLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Ryzee | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Ryzee specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					RyzeeLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= RyzeeLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Ryzee | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Ryzee specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						RyzeeLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Ryzee | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Ryzee specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					RyzeeLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			RyzeeLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			RyzeeLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Ryzee had an issue creating settings.') end
...
function RyzeeLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function RyzeeLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function RyzeeLibrary:Destroy()
	RyzeeDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Ryzee Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "RyzeeOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function RyzeeLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RyzeeLibrary:Notify({Title = "Ryzee Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RyzeeLibrary:Notify({Title = "Ryzee Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Ryzee Configurations Error | '..tostring(result))
			RyzeeLibrary:Notify({Title = "Ryzee Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = RyzeeLibrary:CreateWindow({
	--	Name = "Ryzee Example Window",
	--	LoadingTitle = "Ryzee Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using Ryzee may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Ryzee to get the key from
	...
	----RyzeeLibrary:Notify({Title = "Ryzee Interface", Content = "Welcome to Ryzee. These - are the brand new notification design for Ryzee, with custom sizing and Ryzee calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(RyzeeLibrary.Theme) do
...
task.delay(4, function()
	RyzeeLibrary.LoadConfiguration()
